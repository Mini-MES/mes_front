...
* **테스트 내용**:
  1. `작업지시 등록 폼`에서 `원자재-A` 50개 생산 지시 발행.
  2. **재고 충분 시**: LOT 번호가 자동 발급되고 상태가 `RELEASED` ➡️ `WIP`로 전환되는지 확인.
  3. **재고 부족 예외 테스트**: 현재 재고(10개)보다 많은 수량(50개) 지시 발행 시도.
* **통과 기준**:
...
#### 🧪 예외 시험 A: 양품 수량 0개 시 공정 이송 시도
* **시도**: 양품 수량에 `0` 입력 후 `다음 공정 이송` 버튼 클릭.
* **기대 결과**: 프론트엔드 버튼이 비활성화되거나 경고 팝업이 뜨며 이송이 차단됨.

#### 🧪 예외 시험 B: 불량 발생 및 HOLD 보류 전환
...
* **기대 결과**: 
  ...
  * 백엔드 API로 강제 이송 요청을 보내도 `400 Bad Request` 에러로 차단됨.

#### 🧪 예외 시험 C: 관리자 품질 승인 / 보류 해제
