...
- 시연 자료: [작업지시 생성](../src/docs/video/CreateWorkOrder.mp4)

### AC-02. 자재 재고 및 부족 알림

관리자가 원자재를 등록하거나 수량·안전재고를 변경한 뒤, 부족한 자재의 상태를 확인한다.

- 기대 결과: 변경한 재고가 자재 현황과 분석 화면에 반영된다.
...
- 확인 항목: 선택 설비만 해당 LOT을 점유하는지, Counter 증가량이 양품 수량에 반영되는지, 온도·설비 상태가 실시간 표시되는지.
- 시연 자료: [생산 시작](../src/docs/video/StartProduction.mp4)

### AC-04. 불량 등록, HOLD 및 재작업

작업자가 불량 수량과 사유를 등록하고, 관리자가 LOT 보류를 해제한 뒤 재작업을 진행한다.

- 기대 결과: 불량 실적과 사유가 저장되고 LOT이 HOLD로 전환된다. 해제 후에는 공정 작업을 재개할 수 있다.
- 확인 항목: 불량 수량·사유 코드, HOLD 알림, HOLD 중 공정 이동 차단, 보류 해제 후 LOT 상태.
- 시연 자료: [재작업 및 보류 해제](../src/docs/video/ReWork.mp4)
...
- 시연 자료: [다공정 생산](../src/docs/video/MultiProcess.mp4)
...
- 시연 자료: [비가동 발생](../src/docs/video/DownTime.mp4), [비가동 사유 등록](../src/docs/video/DowntimeReason.mp4)
...
- 시연 자료: [작업지시 완료](../src/docs/video/CompleteOrder.mp4)
...
- 시연 자료: [출하 처리](../src/docs/video/Shipment.mp4)
